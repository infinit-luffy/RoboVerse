...
from ._vendor_utils import clip_and_scale_action, normalize_action_space
from .ruckig_utils import (
...
class BaseController:
    ...
    def _initialize_joints(self):
        ...
        try:
            ...
            self.joint_indices = get_active_joint_indices(self.articulation, joint_names)
    ...
    def _clip_and_scale_action(self, action):
        return clip_and_scale_action(action, self._action_space.low, self._action_space.high)
    ...
    def plan_joint_path(self, start_qpos, target_qpos, vlim, alim, jerklim, init_v=0.0):
        ...
        velocity = []
        for t in times:
            p, v, a = planner_result.at_time(t)
            position.append(p)
            velocity.append(v)
        # print(times[0], times[-1], position[0], position[-1], velocity[0], velocity[-1])
...
class DictController(BaseController):
    def __init__(
        ...
        for uid, config in configs.items():
            self.controllers[uid] = config.controller_cls(config, articulation, control_freq, sim_freq=sim_freq)
    ...
    def _initialize_action_space(self):
        ...
        named_spaces = [(uid, controller.action_space) for uid, controller in self.controllers.items()]
    ...
    def _assert_fully_actuated(self):
        ...
        if len(active_joints) != len(self.joints) or set(active_joints) != set(self.joints):
            ...
            raise AssertionError(f"{self.articulation} is not fully actuated")

    def set_drive_property(self):
        raise RuntimeError("Undefined behaviors to set drive property for multiple controllers")
