...
@configclass
class KettleCfg(BaseRLTaskCfg):
    """class for pour water tasks."""
    ...
    objects_cfg = {
        "kettle": ArticulationObjCfg(
            name="kettle",
            urdf_path="roboverse_data/assets/bidex/objects/urdf/kettle_mobility/kettle_mobility.urdf",
            usd_path="roboverse_data/assets/bidex/objects/usd/kettle_mobility/kettle_mobility.usd",
        ...
        "bucket": RigidObjCfg(
            name="bucket",
            urdf_path="roboverse_data/assets/bidex/objects/urdf/bucket_mobility/bucket_mobility.urdf",
            usd_path="roboverse_data/assets/bidex/objects/usd/bucket_mobility/bucket_mobility.usd",
            default_density=500.0,
            collapse_fixed_joints=True,
            use_vhacd=True,
            override_com=True,
    ...
    def set_init_states(self) -> None:
        """Set the initial states for the shadow hand pour water task."""
        self.state_shape = 0
        for robot in self.robots:
    ...
    def observation_fn(self, envstates: list[DictEnvState], actions: torch.Tensor, device=None) -> torch.Tensor:
        ...
        Returns:
            ...
            bucket pose
            bucket linear velocity
            bucket angle velocity
            kettle handle position (x, y, z)
            bucket handle position (x, y, z)
        ...
        t += self.action_shape // 2
        if self.use_prio:
            state_obs[:, t : t + 13] = envstates.objects["kettle"].root_state
            state_obs[:, t + 10 : t + 13] *= self.vel_obs_scale  # object1 angvel
            t += 13
            state_obs[:, t : t + 13] = envstates.objects["bucket"].root_state
            state_obs[:, t + 10 : t + 13] *= self.vel_obs_scale  # object2 angvel
            t += 13
            ...
            state_obs[:, t : t + 3] = kettle_handle_pos
            t += 3
            state_obs[:, t : t + 3] = bucket_handle_pos
            t += 3
        obs["state"] = state_obs
    ...
    def reward_fn(
        ...
        kettle_spout_pos = envstates.objects["kettle"].root_state[:, :3]
        kettle_spout_rot = envstates.objects["kettle"].root_state[:, 3:7]
        kettle_spout_pos = kettle_spout_pos + math.quat_apply(kettle_spout_rot, self.x_unit_tensor * -0.2)
        kettle_spout_pos = kettle_spout_pos + math.quat_apply(kettle_spout_rot, self.z_unit_tensor * 0.07)
...
def compute_task_reward(
    ...
    success = torch.norm(bucket_handle_pos - kettle_spout_pos, p=2, dim=-1) < 0.05
