...
class LiberoKitchenScene10PutButterAtFrontInTopDrawerAndCloseTask(Libero90BaseTask):
    ...
    Task Description:
    - Put butter_1 at the front into the top drawer of the wooden cabinet
    - Close the top drawer of the wooden cabinet

    This is a compound manipulation task that requires:
    1. Picking up the butter_1 from the table
    2. Placing the butter_1 inside the top drawer
    3. Closing the top drawer

    Objects:
    - akita_black_bowl (object): Distractor object
    - chocolate_pudding (object): Distractor object
    - butter_1 (object): Butter to be placed in the drawer
    - butter_2 (object): Distractor object
    - wooden_cabinet (fixture): Cabinet with drawers

    Goal: (In butter_1 wooden_cabinet_1_top_region) AND (Close wooden_cabinet_1_top_region)
    """
    ...
    task_desc = "Put the butter at the front in the top drawer of the cabinet and close it (scene10)"

    workspace_name = ("kitchen_table",)
    workspace_offset = ((0, 0, 0),)
    workspace_size = ((1.0, 1.2, 0.05),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_kitchen_scene10_put_the_butter_at_the_front_in_the_top_drawer_of_the_cabinet_and_close_it_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker: butter_1 is inside the top drawer AND top drawer is closed."""
        # 1. check if butter_1 is in the top drawer
        butter_pos = states.objects["butter_1"].root_state[:, :3]  # (N,3)
