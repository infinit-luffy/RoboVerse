...
class SceneRandomCfg:
    ...
    def __post_init__(self):
        ...
        enabled_elements = [
            elem for elem in [self.floor, self.walls, self.ceiling, self.table]
            if elem is not None and elem.enabled
        ]
...
class SceneRandomizer(BaseRandomizerType):
    ...
    def _create_or_update_floor(self, env_prim_path: str, env_id: int):
        ...
        if self.cfg.floor.material_randomization and self.cfg.floor_materials is not None:
            material_path = self._select_material(
                self.cfg.floor_materials,
                "floor_index"
            )
            if material_path:
    ...
    def _create_or_update_walls(self, env_prim_path: str, env_id: int):
        ...
        wall_configs = self._generate_wall_configs(
            self.cfg.walls.size,
            self.cfg.walls.position
        )
    ...
    def _create_or_update_ceiling(self, env_prim_path: str, env_id: int):
        ...
        if ceiling_path not in self._created_prims:
            self._create_cube_prim(
                ceiling_path,
                self.cfg.ceiling.size,
                self.cfg.ceiling.position
            )
    ...
    def _select_material(
        ...
        if material_pool.selection_strategy == "random":
            ...
            return self._rng.choices(
                material_pool.material_paths,
                weights=material_pool.weights,
                k=1
            )[0]
            ...
            for mesh_path in mesh_prims_paths:
                ...
                if mesh_prim and ("ground" in mesh_path.lower() or "terrain" in mesh_path.lower()):
                    ...
                    try:
                        ...
                        logger.info(f"Successfully ensured UV for terrain mesh")
                    except Exception as e:
