...
@configclass
class TwoCatchUnderarmCfg(BaseRLTaskCfg):
    """class for bidex two catch underarm tasks."""
    ...
    objects_cfg = {
        "cube": RigidObjCfg(
            ...
            usd_path="roboverse_data/assets/bidex/objects/usd/cube_multicolor/cube_multicolor.usd",
        ...
        "egg": RigidObjCfg(
            ...
            usd_path="roboverse_data/assets/bidex/open_ai_assets/usd/egg/egg.usd",
    ...
    def set_objects(self) -> None:
        ...
        if self.current_robot_type == "shadow":
            ...
            self.robot_init_state = {
                "right_hand": {
                    "pos": torch.tensor([0.0, 0.086, 0.0]),
                    "rot": torch.tensor([0.7071, 0, 0, -0.7071]),
                    "dof_pos": {
                        "joint_0": 0.0,
                        "joint_1": 0.0,
    ...
    def reset_init_pose_fn(self, init_states: list[DictEnvState], env_ids: torch.Tensor) -> list[DictEnvState]:
        ...
        if isinstance(init_states, list):
            ...
            rand_floats = math.torch_rand_float(-1.0, 1.0, (len(env_ids), num_dofs + 10), device="cpu")

            new_right_object_rot = randomize_rotation(rand_floats[:, 6], rand_floats[:, 7], x_unit_tensor, y_unit_tensor)
            new_left_object_rot = randomize_rotation(
                rand_floats[:, 8], rand_floats[:, 9], x_unit_tensor, y_unit_tensor
            )
            ...
            # generate random values
            rand_floats = math.torch_rand_float(-1.0, 1.0, (len(env_ids), num_dofs + 10), device=self.device)
