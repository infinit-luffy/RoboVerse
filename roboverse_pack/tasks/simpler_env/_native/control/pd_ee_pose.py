...
from ._vendor_utils import clip_and_scale_action, get_entity_by_name, vectorize_pose
...
class PDEEPosController(PDJointPosController):
    ...
    def _initialize_joints(self):
        ...
        if self.config.ee_link:
            self.ee_link = get_entity_by_name(self.articulation.get_links(), self.config.ee_link)
        else:
...
class PDEEPoseController(PDEEPosController):
    ...
    def _clip_and_scale_action(self, action):
        ...
        pos_action = clip_and_scale_action(action[:3], self._action_space.low[:3], self._action_space.high[:3])
        rot_action = action[3:]
    ...
    def compute_target_pose(self, prev_ee_pose_at_base, action):
        if self.config.use_delta:
            ...
            if self.config.frame == "base":
                ...
                target_pose = (
                    sapien.Pose(p=cur_ee_pose_at_base.p) * delta_pose * sapien.Pose(p=cur_ee_pose_at_base.p).inv()
