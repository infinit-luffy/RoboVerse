"""Configuration for the Libero kitchen scene5 put the black bowl in the top drawer of the cabinet task."""
...
class LiberoKitchenScene5PutBlackBowlInTopDrawerTask(Libero90BaseTask):
    """Configuration for the Libero kitchen scene5 put the black bowl in the top drawer of the cabinet task."""
    ...
    task_desc = "Put the black bowl in the top drawer of the cabinet (scene5)"

    workspace_name = ("kitchen_table",)
    workspace_offset = ((0, 0, 0),)
    workspace_size = ((1.0, 1.2, 0.05),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_kitchen_scene5_put_the_black_bowl_in_the_top_drawer_of_the_cabinet_traj_v2.pkl"
    ...
    def _terminated(self, states: TensorState) -> torch.Tensor:
        ...
        bbox_relative_pos = torch.tensor([0.00328, 0.01128 + bbox_displacement, 0.18563], device=bowl_pos.device)
        ...
        bbox_half_size = torch.tensor([0.02993, 0.07561, 0.10224], device=bowl_pos.device)
        bbox_T_world = torch.matmul(cabinet_T, bbox_relative_T)
        R = bbox_T_world[:, :3, :3]
        t = bbox_T_world[:, :3, 3]
        bowl_local = torch.matmul(R.transpose(1, 2), (bowl_pos - t).unsqueeze(-1)).squeeze(-1)
        eps = 1e-6
        inside = (bowl_local.abs() <= (bbox_half_size.unsqueeze(0) + eps)).all(dim=-1)
        return inside
