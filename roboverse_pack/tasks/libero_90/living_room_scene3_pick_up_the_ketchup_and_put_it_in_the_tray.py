"""Configuration for the Libero living room scene3 pick up the ketchup and put it in the tray task."""
...
class LiberoLivingRoomScene3PickUpKetchupTask(Libero90BaseTask):
    ...
    Task Description:
    - Pick up the ketchup from the table
    - Place the ketchup inside the tray/contain_region

    This is a compound manipulation task that requires:
    1. Picking up the ketchup from the table
    2. Placing the ketchup inside the tray/contain_region
    ...
    Goal: Place ketchup inside wooden_tray.
    """
    ...
    max_episode_steps = 238
    task_desc = "Pick up the ketchup and put it in the tray (living_room_scene3)"

    workspace_name = ("living_room_table",)
    workspace_offset = ((0.0, 0, 0.42),)
    workspace_size = ((1.0, 1.2, 0.1),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_living_room_scene3_pick_up_the_ketchup_and_put_it_in_the_tray_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker: ketchup is inside wooden_tray/contain_region bounding box."""
        obj_pos = states.objects["ketchup"].root_state[:, :3]
        N = obj_pos.shape[0]
