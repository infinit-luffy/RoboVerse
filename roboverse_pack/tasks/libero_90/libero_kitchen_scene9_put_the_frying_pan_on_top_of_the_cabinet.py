"""Configuration for the Libero kitchen scene9 put the frying pan on top of the cabinet task."""
...
class LiberoKitchenScene9PutTheFryingPanOnTopOfTheCabinetTask(Libero90BaseTask):
    """Configuration for the Libero kitchen scene9 put the frying pan on top of the cabinet task.

    Task Description:
    - Put the chefmate 8 frying pan on top of the wooden two layer shelf
    ...
    - chefmate_8_frypan (object): Frying pan to be placed on top of the cabinet
    - wooden_two_layer_shelf (fixture): Target shelf with two layers
    - flat_stove (fixture): Distractor stove

    Goal: (On chefmate_8_frypan wooden_two_layer_shelf_top_side)
    """
    ...
    task_desc = "Put the frying pan on top of the cabinet (scene9)"
    ...
    traj_filepath = (
        "roboverse_data/trajs/libero90/libero_90_kitchen_scene9_put_the_frying_pan_on_top_of_the_cabinet_traj_v2.pkl"
    )

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker: frying pan is on top of the cabinet (top_side surface).

        Success condition (similar to wine_rack in kitchen4):
        - Frying pan is within the bounding box region defined by the top_side site
        ...
        bbox_lower = torch.tensor([-0.12534, -0.09438, 0], device=frypan_pos.device)
        bbox_upper = torch.tensor([0.12534, 0.09438, 0.03], device=frypan_pos.device)
