...
class HandOverCfg(BaseRLTaskCfg):
    ...
    def set_objects(self) -> None:
        """Set the objects for the hand over task."""
        ...
        if self.current_robot_type == "shadow":
            ...
            self.robot_init_state = {
                ...
                "left_hand": {
                    "pos": torch.tensor([0.0, -1.296, 0.0]),
                    "rot": torch.tensor([0.7071, 0, 0, 0.7071]),
                    "dof_pos": {
                        "FFJ1": 0.0,
                        "FFJ2": 0.0,
            ...
            self.robot_init_state = {
                "right_hand": {
                    "pos": torch.tensor([0.0, 0.085, 0.0]),
                    "rot": torch.tensor([0.7071, 0, 0, -0.7071]),
                    "dof_pos": {
                        "joint_0": 0.0,
                        "joint_1": 0.0,
    ...
    def set_init_states(self) -> None:
        ...
        self.init_goal_pos = torch.tensor(
            [0.0, -0.64, 0.84], dtype=torch.float32, device=self.device
    ...
    def observation_fn(self, envstates: list[DictEnvState], actions: torch.Tensor, device=None) -> torch.Tensor:
        """Observation function for hand over tasks.
