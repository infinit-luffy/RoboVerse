...
class SwingCupCfg(BaseRLTaskCfg):
    ...
    objects_cfg = {
        "cup": ArticulationObjCfg(
            name="cup",
            urdf_path="roboverse_data/assets/bidex/objects/urdf/cup_mobility/cup_mobility.urdf",
            usd_path="roboverse_data/assets/bidex/objects/usd/cup_mobility/cup_mobility.usd",
    ...
    def observation_fn(self, envstates: list[DictEnvState], actions: torch.Tensor, device=None) -> torch.Tensor:
        ...
        if self.use_prio:
            ...
            state_obs[:, t : t + 3] = cup_right_handle_pos
            t += 3
            state_obs[:, t : t + 3] = cup_left_handle_pos
            t += 3
            state_obs[:, t : t + 4] = self.goal_rot
            t += 4
        obs["state"] = state_obs
    ...
    def reward_fn(
        ...
        right_hand_reward, right_hand_dist = self.robots[0].reward(cup_right_handle_pos)
        left_hand_reward, left_hand_dist = self.robots[1].reward(cup_left_handle_pos)
...
def compute_task_reward(
    ...
    Args:
        ...
        right_hand_dist (tensor): The distance between the right hand and the right handle of the cup

        left_hand_dist (tensor): The distance between the left hand and the left handle of the cup
    ...
    up_rew = torch.where(right_hand_dist <= 0.08, torch.where(left_hand_dist <= 0.08, rot_rew, up_rew), up_rew)
