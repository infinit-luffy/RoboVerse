...
class IsaacgymHandler(BaseSimHandler):
    def __init__(self, scenario: ScenarioCfg, optional_queries: dict[str, BaseQueryType] | None = None):
        ...
        if hasattr(scenario, "device"):
            self._device = torch.device(scenario.device)
        else:
        ...
        self._robot_link_dict_list: list = []  # dict of robot link index dict
        self._robot_joint_dict_list: list = []  # dict of robot joint index dict
    ...
    def _load_robot_assets(self) -> None:
        ...
        for robot in self.robots:
            ...
            _manual_pd_on = (
                any(mode == "effort" for mode in robot.control_type.values())
                if robot.control_type
                else self._manual_pd_on
            )
            self._manual_pd_on = _manual_pd_on or self._manual_pd_on
            ...
            for i, dof_name in enumerate(dof_names):
                ...
                if i_control_mode == "effort":
                    ...
                    robot_dof_props["stiffness"][i] = (
                        i_actuator_cfg.stiffness if i_actuator_cfg.stiffness is not None else 800.0
                    )
                    robot_dof_props["damping"][i] = (
                        i_actuator_cfg.damping if i_actuator_cfg.damping is not None else 40.0
                    )
            ...
            self._robot_joint_dict_list.append(self.gym.get_asset_dof_dict(robot_asset))
    ...
    def _make_envs(
        ...
        for i in range(self.num_envs):
            ...
            for robot_idx, robot_info in enumerate(
                ...
                # self.gym.set_actor_rigid_shape_properties(env, robot_handle, robot_rigid_shape_props_asset)
    ...
    def _get_states(self, env_ids: list[int] | None = None) -> list[DictEnvState]:
        ...
        return TensorState(
            objects=object_states, robots=robot_states, cameras=camera_states, sensors=sensor_states, extras=extras
        )
        ...
        # if states is TensorState, reindex the tensors and set state
            ...
            for obj_id, obj in enumerate(self.objects):
                ...
                if isinstance(obj, ArticulationObjCfg) and len(self._joint_info[obj.name]["names"]) > 0:
                    ...
                    new_dof_states[env_ids_tensor.unsqueeze(1), joint_ids_reindex.unsqueeze(0), 1] = joint_vel[
                        env_ids, :
                    ].clone()
            for robot_id, robot in enumerate(self.robots):
