...
from roboverse_pack.robots.franka_allegro_left_cfg import FrankaAllegroHandLeftCfg
...
class TurnButtonCfg(BaseRLTaskCfg):
    ...
    objects_cfg = {
        "button": ArticulationObjCfg(
            name="button",
            scale=(1, 1, 1),
            urdf_path="roboverse_data/assets/bidex/objects/urdf/switch_mobility/switch_mobility.urdf",
            usd_path="roboverse_data/assets/bidex/objects/usd/switch_mobility/switch_mobility.usd",
    ...
    def set_sim_params(self, sim_type=None) -> None:
        ...
        if sim_type is not None:
            self.sim = sim_type
        if self.sim in ["isaacgym"]:
            self.sim_params.dt = 1.0 / 60.0
            self.sim_params.substeps = 2
            self.sim_params.num_threads = 4
            self.decimation = 1
        elif self.sim in ["isaacsim"]:
    ...
    def set_objects(self) -> None:
        ...
        self.action_shape = 0
        for robot in self.robots:
            if robot.hand_controller == "ik":
                self.action_shape += 6 * robot.num_fingertips
            elif robot.hand_controller == "dof_pos":
                self.action_shape += robot.num_actuated_joints - robot.num_arm_joints
            if robot.arm_controller in ["ik"]:
                self.action_shape += 6
            elif robot.arm_controller in ["ik_abs"]:
                self.action_shape += 7
            elif robot.arm_controller in ["dof_pos"]:
                self.action_shape += robot.num_arm_joints
        for name in self.robots[0].fingertips:
            r_name = "right" + name
            self.sensors.append(ForceTorqueSensorCfg(base_link=(self.robots[0].name, name), name=r_name))
        for name in self.robots[1].fingertips:
            l_name = "left" + name
            self.sensors.append(ForceTorqueSensorCfg(base_link=(self.robots[1].name, name), name=l_name))
    ...
    def scale_action_fn(self, actions: torch.Tensor) -> torch.Tensor:
        ...
        for robot in self.robots:
            if robot.arm_controller == "dof_pos":
                dpose = actions[:, actions_start : actions_start + robot.num_arm_joints]
                if robot.use_relative_control:
                    dpose = dpose * self.sim_params.dt * self.decimation
                arm_dof_targets = robot.control_arm(dpose, dpose.shape[0], dpose.device)
                actions_start += robot.num_arm_joints
            elif robot.arm_controller in ["ik"]:
    ...
    def observation_fn(self, envstates: list[DictEnvState], actions: torch.Tensor, device=None) -> torch.Tensor:
    ...
    def reset_init_pose_fn(self, init_states: list[DictEnvState], env_ids: torch.Tensor) -> list[DictEnvState]:
        ...
        """
        if self.reset_dof_pos_noise == 0.0 and self.reset_position_noise == 0.0 and not self.use_high_randomness:
        ...
        if isinstance(init_states, list):
            ...
            z_unit_tensor = torch.tensor([0, 0, 1], dtype=torch.float, device="cpu").repeat((len(env_ids), 1))

            # generate random values
            rand_floats = math.torch_rand_float(-1.0, 1.0, (len(env_ids), num_dofs + 8), device="cpu")

            new_left_object_rot = randomize_rotation(rand_floats[:, 6], rand_floats[:, 6] * 0, z_unit_tensor, y_unit_tensor)
            new_right_object_rot = randomize_rotation(rand_floats[:, 7], rand_floats[:, 7] * 0, z_unit_tensor, y_unit_tensor)

            x_range = (-0.25, 0.25)
            left_y_range = (-0.4, -0.1)
            right_y_range = (0.1, 0.4)
            left_obj_init_x = torch.rand(len(env_ids)) * (x_range[1] - x_range[0]) + x_range[0]
            right_obj_init_x = torch.rand(len(env_ids)) * (x_range[1] - x_range[0]) + x_range[0]
            left_obj_init_y = torch.rand(len(env_ids)) * (left_y_range[1] - left_y_range[0]) + left_y_range[0]
            right_obj_init_y = torch.rand(len(env_ids)) * (right_y_range[1] - right_y_range[0]) + right_y_range[0]
            ...
            for i, env_id in enumerate(env_ids):
                ...
                for obj_name in reset_state[env_id]["objects"].keys():
                    if not self.use_high_randomness:
                        if obj_name == f"{self.current_object_type}_1":
                            reset_state[env_id]["objects"][obj_name]["pos"][:3] += (
                                self.reset_position_noise * rand_floats[i, :3]
                            )
                            reset_state[env_id]["objects"][obj_name]["rot"] = new_right_object_rot[i]
                        elif obj_name == f"{self.current_object_type}_2":
                            reset_state[env_id]["objects"][obj_name]["pos"][:3] += (
                                self.reset_position_noise * rand_floats[i, 3:6]
                            )
                            reset_state[env_id]["objects"][obj_name]["rot"] = new_left_object_rot[i]
                    else:
                        if obj_name == f"{self.current_object_type}_1":
                            reset_state[env_id]["objects"][obj_name]["pos"][0] = right_obj_init_x[i]
                            reset_state[env_id]["objects"][obj_name]["pos"][1] = right_obj_init_y[i]
                            reset_state[env_id]["objects"][obj_name]["rot"] = new_right_object_rot[i]
                        elif obj_name == f"{self.current_object_type}_2":
                            reset_state[env_id]["objects"][obj_name]["pos"][0] = left_obj_init_x[i]
                            reset_state[env_id]["objects"][obj_name]["pos"][1] = left_obj_init_y[i]
                            reset_state[env_id]["objects"][obj_name]["rot"] = new_left_object_rot[i]
            ...
            z_unit_tensor = torch.tensor([0, 0, 1], dtype=torch.float, device=self.device).repeat((len(env_ids), 1))

            # generate random values
            rand_floats = math.torch_rand_float(-1.0, 1.0, (len(env_ids), num_dofs + 8), device=self.device)
            ...
            left_y_range = (-0.4, -0.1)
            right_y_range = (0.1, 0.4)
            left_obj_init_x = torch.rand(len(env_ids), device=self.device) * (x_range[1] - x_range[0]) + x_range[0]
            right_obj_init_x = torch.rand(len(env_ids), device=self.device) * (x_range[1] - x_range[0]) + x_range[0]
            left_obj_init_y = torch.rand(len(env_ids), device=self.device) * (left_y_range[1] - left_y_range[0]) + left_y_range[0]
            right_obj_init_y = torch.rand(len(env_ids), device=self.device) * (right_y_range[1] - right_y_range[0]) + right_y_range[0]
            
            for obj_id, obj in enumerate(self.objects):
                root_state = reset_state.objects[obj.name].root_state
                if not self.use_high_randomness:
                    if obj.name == f"{self.current_object_type}_1":
                        root_state[env_ids, :3] += self.reset_position_noise * rand_floats[:, :3]
                        root_state[env_ids, 3:7] = new_right_object_rot
                    elif obj.name == f"{self.current_object_type}_2":
                        root_state[env_ids, :3] += self.reset_position_noise * rand_floats[:, 3:6]
                        root_state[env_ids, 3:7] = new_left_object_rot
                else:
                    if obj.name == f"{self.current_object_type}_1":
                        root_state[env_ids, 0] = right_obj_init_x
                        root_state[env_ids, 1] = right_obj_init_y
                        # root_state[env_ids, 3:7] = new_right_object_rot
                    elif obj.name == f"{self.current_object_type}_2":
                        root_state[env_ids, 0] = left_obj_init_x
                        root_state[env_ids, 1] = left_obj_init_y
                        # root_state[env_ids, 3:7] = new_left_object_rot
