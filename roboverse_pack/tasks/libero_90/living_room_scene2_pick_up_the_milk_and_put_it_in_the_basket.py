"""Configuration for the Libero living room scene2 pick up the milk and put it in the basket task."""
...
class LiberoLivingRoomScene2PickUpMilkTask(Libero90BaseTask):
    ...
    - Place the milk inside the basket/contain_region

    This is a compound manipulation task that requires:
    1. Picking up the milk from the table
    2. Placing the milk inside the basket/contain_region
    ...
    Goal: Place milk inside basket/contain_region.
    """
    ...
    task_desc = "Pick up the milk and put it in the basket (living_room_scene2)"

    workspace_name = ("living_room_table",)
    workspace_offset = ((0, 0, 0),)
    workspace_size = ((1.0, 1.2, 0.1),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_living_room_scene2_pick_up_the_milk_and_put_it_in_the_basket_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker: milk is inside basket/contain_region bounding box."""
        obj_pos = states.objects["milk"].root_state[:, :3]
        N = obj_pos.shape[0]
